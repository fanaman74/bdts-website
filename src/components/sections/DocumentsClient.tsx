...
// Distinct colour per company (bg + text Tailwind classes)
...
function companyColor(name: string): string {
  return COMPANY_COLOR[name] ?? "bg-navy text-white";
}
...
function DocumentRow({ doc, onChat, askLabel, openLabel }: { doc: Document; onChat: () => void; askLabel: string; openLabel: string }) {
  ...
  return (
    <div className="group flex items-center gap-4 px-4 py-4 rounded-lg bg-white border border-border hover:border-gold/40 hover:shadow-sm hover:shadow-gold/10 transition-all duration-150 hover:translate-x-0.5">
      ...
      {/* Company badge */}
      <span className={`hidden sm:inline-flex flex-shrink-0 items-center justify-center min-w-[110px] px-2.5 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${compColor}`}>
        {doc.company}
      </span>
